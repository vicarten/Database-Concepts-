...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `commissionCheck`(
    IN employeeNum INT,
...
/*!50001 VIEW `customerview` AS select `customer`.`firstName` AS `firstName`,`customer`.`lastName` AS `lastname`,`customer`.`address` AS `address`,`citystate`.`city` AS `city`,`citystate`.`state` AS `state`,`citystate`.`zipCode` AS `zipCode`,`customer`.`phone` AS `phone`,`customer`.`email` AS `email` from (`customer` join `citystate`) where (`customer`.`zipCode` = `citystate`.`zipCode`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `employeeview` AS select `employee`.`firstname` AS `firstName`,`employee`.`lastname` AS `lastname`,`employee`.`address` AS `address`,`citystate`.`city` AS `city`,`citystate`.`state` AS `state`,`citystate`.`zipCode` AS `zipCode`,`employee`.`phone` AS `phone`,`employee`.`email` AS `email` from (`employee` join `citystate`) where (`employee`.`zipCode` = `citystate`.`zipCode`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `saleview` AS select concat(`customer`.`firstName`,' ',`customer`.`lastName`) AS `Customer Name`,concat(`customer`.`address`,' ',`citystate`.`city`,' ',`citystate`.`state`,' ',`citystate`.`zipCode`) AS `Customer Address`,`customer`.`phone` AS `Customer Phone`,`customer`.`email` AS `Customer Email`,concat(`employee`.`firstname`,' ',`employee`.`lastname`) AS `Sales Associate`,`employee`.`phone` AS `Sales Associate Phone`,`employee`.`email` AS `Sales Associate Email`,`vehicle`.`year` AS `Year`,`make`.`make` AS `Make`,`model`.`model` AS `Model`,`color`.`color` AS `Color`,`type`.`type` AS `Type`,`vehicle`.`vin` AS `VIN`,`sale`.`salePrice` AS `Sale Price` from ((((((((`customer` join `citystate`) join `employee`) join `vehicle`) join `make`) join `model`) join `color`) join `type`) join `sale`) where ((`customer`.`ID` = `sale`.`customerId`) and (`customer`.`zipCode` = `citystate`.`zipCode`) and (`employee`.`ID` = `sale`.`employeeId`) and (`vehicle`.`ID` = `sale`.`vehicleId`) and (`model`.`ID` = `vehicle`.`model`) and (`color`.`ID` = `vehicle`.`color`) and (`type`.`ID` = `vehicle`.`type`) and (`make`.`ID` = `vehicle`.`make`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2022-12-05 22:05:56
